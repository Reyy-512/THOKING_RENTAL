            ...
            <div class="row">
                ...
                <div class="col-lg-6 mb-4">
                    ...
                    <div class="card shadow-sm mt-4">
                        <div class="card-header bg=#0056b3 text-white">
                            <h5>Merk Mobil : <?= htmlspecialchars($mobil['merk']) ?></h5>
                        </div>
...
<!-- Form ubah status -->
                        <form method="post" action="proses.php?id=konfirmasi">
                            ...
                            <select name="status" class="form-control" onchange="toggleNotaButton(this.value)">
                                ...
                                <option <?= $hasil['konfirmasi_pembayaran'] == 'Pembayaran ditolak' ? 'selected' : '' ?>>Pembayaran ditolak</option>
                            </select>
                        ...
                        <!-- Tombol kirim nota -->
                        <div id="btn-nota" class="mt-2">
                            <?php if ($hasil['konfirmasi_pembayaran'] == 'Pembayaran di terima'): ?>
                                <a href="kirim_nota.php?id=<?= urlencode($hasil['id_booking']) ?>&jenis=diterima" 
                                class="btn btn-success btn-block" 
                                onclick="return confirm('Kirim nota booking diterima ke pengguna?');">
                                <i class="fa fa-paper-plane"></i> Kirim Nota Diterima
                                </a>
                            <?php elseif ($hasil['konfirmasi_pembayaran'] == 'Pembayaran ditolak'): ?>
                                <a href="kirim_nota.php?id=<?= urlencode($hasil['id_booking']) ?>&jenis=ditolak" 
                            class="btn btn-danger btn-block mt-2" 
                            onclick="return confirm('Kirim nota booking ditolak ke pengguna?');">
                            <i class="fa fa-times"></i> Kirim Nota Penolakan
                        </a>
                            <?php endif; ?>
                        </div>

                        <script>
                        function toggleNotaButton(value) {
                            let container = document.getElementById('btn-nota');
                            let id_booking = "<?= urlencode($hasil['id_booking']) ?>";
                            let btnHTML = '';

                            if (value === 'Pembayaran di terima') {
                                btnHTML = `<a href="kirim_nota.php?id=${id_booking}&jenis=diterima" 
                                            class="btn btn-success btn-block" 
                                            onclick="return confirm('Kirim nota booking diterima ke pengguna?');">
                                            <i class="fa fa-paper-plane"></i> Kirim Nota Diterima
                                        </a>`;
                            } else if (value === 'Pembayaran ditolak') {
                                btnHTML = `<a href="kirim_nota.php?id=${id_booking}&jenis=ditolak" 
                                            class="btn btn-danger btn-block mt-2" 
                                            onclick="return confirm('Kirim nota booking ditolak ke pengguna?');">
                                            <i class="fa fa-times"></i> Kirim Nota Ditolak
                                        </a>`;
                            }
